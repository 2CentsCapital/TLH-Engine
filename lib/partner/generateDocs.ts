...
function brandHeader(ws: Worksheet, lastCol: number, docTitle: string, subtitle: string, client: Client) {
  ...
  banner.value = {
    richText: [
      { text: `${P.name} `, font: { bold: true, size: 18, color: { argb: hexToArgb(P.color) }, name: "Calibri" } },
...
function disclaimer(ws: Worksheet, startRow: number, lastCol: number) {
  const colLetter = ws.getColumn(lastCol).letter;
  const lines = [
    `Disclaimer — illustrative co-branded report generated by ${getActivePartner().name} × Valura.`,
...
export async function generateDoc(client: Client, type: DocType) {
  ...
  a.href = url;
  a.download = `${P.name.replace(/\s+/g, "")}-Valura_${FILENAMES[type]}_${client.name.replace(/\s+/g, "-")}_FY25-26.xlsx`;
