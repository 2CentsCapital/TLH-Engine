/**
 * Narnolia × Valura — co-branded tax document generator.
...
/* ── Brand palette (ARGB) ── */
const ORANGE = "FFE0342A";   // Narnolia red
const GREEN = "FF05A049";    // Valura
const NAVY = "FF00111B";
const GREEN_SOFT = "FFEDFAF3";
const ORANGE_SOFT = "FFFDECEA";
const GREY = "FF6B7280";
...
function brandHeader(ws: Worksheet, lastCol: number, docTitle: string, subtitle: string, client: Client) {
  ...
  banner.value = {
    richText: [
      { text: "Narnolia ", font: { bold: true, size: 18, color: { argb: ORANGE }, name: "Calibri" } },
...
function disclaimer(ws: Worksheet, startRow: number, lastCol: number) {
  const colLetter = ws.getColumn(lastCol).letter;
  const lines = [
    "Disclaimer — illustrative co-branded report generated by Narnolia × Valura.",
...
export async function generateDoc(client: Client, type: DocType) {
  ...
  a.href = url;
  a.download = `Narnolia-Valura_${FILENAMES[type]}_${client.name.replace(/\s+/g, "-")}_FY25-26.xlsx`;
